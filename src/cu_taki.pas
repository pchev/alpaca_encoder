...
begin
  y[1, 0] := cos(f) * cos(h) - sin(f) * (z2 / g) + sin(f) * sin(h) * (z1 / g);
  y[2, 0] := sin(f) * cos(h) + cos(f) * (z2 / g) - cos(f) * sin(h) * (z1 / g);
  y[3, 0] := sin(h);
end;

procedure sub_y(f, h: double); // 785
begin
  y[1, 1] := cos(f) * cos(h) + sin(f) * (z2 / g) - sin(f) * sin(h) * (z1 / g);
  y[2, 1] := sin(f) * cos(h) - cos(f) * (z2 / g) + cos(f) * sin(h) * (z1 / g);
  y[3, 1] := sin(h);
end;
...
begin
  ...
  if c = 0 then
    f := 1000;
  if (c <> 0) and (y[1, 1] = 0) and (y[2, 1] > 0) then
    f := 90;
  if (c <> 0) and (y[1, 1] = 0) and (y[2, 1] < 0) then
    f := 270;
  if y[1, 1] > 0 then
    f := arctan(y[2, 1] / y[1, 1]) * g;
  if y[1, 1] < 0 then
    f := arctan(y[2, 1] / y[1, 1]) * g + 180;
  f := f - int(f / 360) * 360;
end;
