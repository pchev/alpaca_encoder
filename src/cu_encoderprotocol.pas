...
uses
  synaser, Dialogs, SysUtils, Classes;
...
begin
  ...
  if (typ=Simulator) or OpenCom(encoder_port, commport, baud, parity, Data, stop, timeout, inttimeout) then
  begin
    port_opened := True;
    encoder_model := model;
    encoder_type := typ;
...
function Encoder_Close: boolean;
begin
  if (encoder_type<>Simulator) then CloseCom(encoder_port);
  port_opened := False;
  encoder_model := '';
  encoder_type := 0;
  Result := True;
...
begin
  msg := '';
  Result := False;
  if (encoder_type<>Simulator) then PurgeBuffer(encoder_port);
  case encoder_type of
    1, 2, 3, 4:
    begin
      buf := 'Q';
